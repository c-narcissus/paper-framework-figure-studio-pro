...
`paper-framework-figure-studio-pro` 是面向计算机科学论文框架图的制图 skill。它的目标是为论文 method overview、architecture diagram、pipeline/process figure 和 agent workflow 提供多样化候选草案，方便作者后续筛选、对照、人工编辑和定稿。
当前 `main` 分支已更新为 **v3.2.15f** 包和示例，该版本是针对 ChatGPT 网页端设计的，只建议用于 ChatGPT 网页端的 chat 和 work 模式下（注意 work 模式下会受 token 额度限制影响），因为 Codex 里的 image gen（text）无法接受图片作为输入，而在 **v3.2.15f** 的 S5 阶段需要依赖 S2 阶段产生的图片作为参考；本周会提供 Cursor 和 ChatGPT chat 模式下的 subagents 协作版本。
此外，本文档架构图依然沿用 **v3.2.15b**。旧版本已归档到对应 Git tags。感谢 Bristol 的刘欣阳同学提供协助。

本周也会提供 Cursor 和 ChatGPT chat 模式下的 subagents 协作版本，仍会侧重独立审计。
...
**v3.2.15c** 修正了 **v3.2.15b** skill安装后无法被codex查找到的问题。此外， **v3.2.15c** 新增了 ACM/IEEE/AAAI 双栏论文 line-art schematic 表面风格，可在在S1/S3结束时的建议提示词基础上，显式人工添加后自动注入到 S2/S5 prompt。添加例子为：“第一轮采用 <ACM/IEEE/AAAI 双栏论文 line-art schematic> 表面风格 ” 。 **v3.2.15c** 还规范了表面风格提醒的位置：风格选项放在非复制提示里。


## 总结

- 当前 `main` 分支使用 **v3.2.15f**，且仅面向 ChatGPT 网页端chat模式和work模式；架构图说明主要沿用 **v3.2.15b**。新版本的生图提示词审核更加严格。
- v3.2.15f延续v3.2.15b把“图生成后再审”的质量控制前移到 S1/S4 的 prompt package：先审 semantic graph、visual render graph、visible text contract、line-carried variables 和 negative constraints，再让 S2/S5 执行生图。
- 建议优先在 ChatGPT 网页端的 chat 模式下使用，因为 work 模式很消耗 token。
- 在网页端会遇到生图卡住问题，一个思路是新开 session，然后根据最近的 checkpoint zip 文件断点继续。但是 checkpoint zip 文件内容可能不全，可以在之前中断的 session 下输入提示词补全 zip 文件，相关提示词见 [使用方式](#使用方式)。**其实，不管 zip 是不是全，都可以用这句，因为使用者哪里知道全不全。** 另外一种解决方案，是找到上一步输入的提示词，重新编辑执行一遍，例如如果 S5 生图卡住，则上翻到输入到 S4 步骤的提示词，重新点击下方的编辑，然后再提交一次。
- 当前 main 的 skill 包是 `paper-framework-figure-studio-pro-v3.2.15f-skill.zip`；由于 Codex 下的 image gen（text）无法接收参考图输入，当前不再发布 Codex 版本；v3.2.15c 包也在项目中。
- 这一版延续v3.2.15b方案，采用 S0-S5 人机协作流程，把论文事实、节点/边/端口、变量位置、可见文字和图文分工先编译成可审计的生图 prompt，再进入生图阶段。
- “契约规范下的随机之美”。觉得效果不好怎么办？ChatGPT 网页端直接在输入 S4/S5 提示词的地方，重新编辑执行就行；底子（生图提示词）好，多试试总会有好的图。都走到这步了，来都来了，就多试几次呗，反正用网页端又不费 token。
- v3.2.15f 仍然保留两轮候选机制：第一轮 S2 做全局探索，默认生成 `C01-C04`；第二轮 S5 做正式候选，默认生成 `F01-F02`。这点和之前版本的“先发散、后收敛”逻辑一致。
- 经过再三思量，第一轮默认风格放弃手绘风，而采用正式出版风格；如果需要手绘风，可以在输入 S2 提示词时声明第一轮为手绘风。之所以这样做，是因为很多时候用户会在第一轮后就开始自己画，手绘风不适合看图画 PPT。
- v3.2.15f 的流程明确收束到 **S5**。S5 之后不再由 skill 自动做最终选择。
- v3.2.15f 仍然不把可编辑 SVG/PPT 作为默认交付目标。默认产物是提供给人工对照复刻的候选参考图；如果需要完全可编辑版本，仍需要后续人工重绘或单独处理。
...
- 旧版本的完整包和完整示例已按版本归档在 Git tags 中，不再放在 main 分支目录下；但是保留了 v3.2.15c 的本地图片与示例目录。因为每个人的审美观不一样，可能有的用户更喜欢之前的版本。
- 整个流程通常都比较慢；如果一开始启动 skill 时模型试图一口气跑完，建议重启 session，并明确要求不要一次跑完整流程。
...
- 在输入 S1 步骤的提示词时，可以在默认的建议提示词基础上追加自己额外的要求，也可以放上自己倾向的一张或多张参考图，并改变默认第一轮推荐图的数量（最多 4 张）。生出候选图方案时会参考这些信息。
...
| S2-SKETCH-EXPLORE          | IMAGE_GENERATION_ONLY            | 按 S1 的 prompt-index 生成第一轮 C01-C04 候选图                         |
...
偏好驱动的第二轮覆盖也必须在 S4 分配阶段落实。如果用户在 S3 指定了 C01、C02这类第一轮候选偏好，S4/S5 需要把它们转化为 formal candidate 的 local-essence refinement，而不是静默丢弃。
...
- 当前 main 的 v3.2.15f skill 包：`paper-framework-figure-studio-pro-v3.2.15f-skill.zip`
- 当前 main 的 v3.2.15f 示例论文与结果目录：`example_semiDFL_v3.2.15f/`
- v3.2.15f 示例论文：`example_semiDFL_v3.2.15f/semiDFL.pdf`
- v3.2.15f ChatGPT 网页端使用 gpt5.5 第一轮结果：`example_semiDFL_v3.2.15f/R2_results_chatgpt_web_v3.2.15_5.5eh/`，`C01.png` 到 `C04.png`
- v3.2.15f ChatGPT 网页端使用 gpt5.5 第二轮结果：`example_semiDFL_v3.2.15f/R1_results_chatgpt_web_v3.2.15_5.5eh/`，`F01.png` 到 `F02.png`
- v3.2.15f ChatGPT 网页端 chat 模式结果（ChatGPT 5.6 Sol，extra high）：`example_semiDFL_v3.2.15f/gptweb_chat_5.6sol_extrahigh/`，`F01.png` 到 `F02.png`
- v3.2.15f ChatGPT 网页端 work 模式结果（ChatGPT 5.6 Sol，extra high）：`example_semiDFL_v3.2.15f/gptweb_work_5.6sol_extrahigh/`，`F01.png`、`F02.png` 和 `conversation.png`
- v3.2.15c skill 包：`paper-framework-figure-studio-pro-v3.2.15c-skill.zip`
- v3.2.15c ACM/IEEE/AAAI line-art 示例结果目录：`3.2.15c_result/`
- v3.2.15c ChatGPT 网页端 ACM/IEEE/AAAI line-art 聊天记录截图：`3.2.15c_result/chatgpt_web_conversation_acm_ieee_aaai_line_art_v3.2.15c.png`
- v3.2.15c S1/S4 额外风格要求标注截图：`3.2.15c_result/S1_prompt_annotated_acm_ieee_aaai_line_art_v3.2.15c.jpg` 和 `3.2.15c_result/S4_prompt_annotated_acm_ieee_aaai_line_art_v3.2.15c.jpg`
- v3.2.15c ChatGPT 网页端第一轮 ACM/IEEE/AAAI line-art 候选：`3.2.15c_result/R1_results_chatgpt_web_acm_ieee_aaai_line_art_v3.2.15c/`，`C01.png` 到 `C08.png`
- v3.2.15c Codex 第一轮 ACM/IEEE/AAAI line-art 候选：`3.2.15c_result/R1_results_codex_acm_ieee_aaai_line_art_v3.2.15c/`，`C01.png` 到 `C08.png`
- v3.2.15c ChatGPT 网页端第二轮 ACM/IEEE/AAAI line-art 候选：`3.2.15c_result/R2_results_chatgpt_web_acm_ieee_aaai_line_art_v3.2.15c/`，`F01.png` 到 `F06.png`
- v3.2.15c Codex 第二轮 ACM/IEEE/AAAI line-art 候选：`3.2.15c_result/R2_results_codex_acm_ieee_aaai_line_art_v3.2.15c/`，`F01.png` 到 `F06.png`
...
ChatGPT 网页版使用时，先把当前 main 中的 `paper-framework-figure-studio-pro-v3.2.15f-skill.zip` 和目标论文 PDF 放进项目 Sources；如需要参考当前示例，可使用 `example_semiDFL_v3.2.15f/semiDFL.pdf`。在需要图像生成的阶段，手动切换到 `Create image`。
...
```text
请严格按照paper-framework-figure-studio-pro-v3.2.15f-skill.zip 里 skill 的人机交互步骤，对example_semiDFL_v3.2.15f/semiDFL.pdf 绘制 diagram。不要查看semiDFL.pdf 里面已有的diagram；这里的“不要查看”不是说不能自己构思出类似图，而是不要被原图先入为主，应根据论文实际内容决定生成或不生成类似结构。
```
...
```text
刚才中断了，请使用 paper-framework-figure-studio-pro v3.2.15f skill 根据当前状态，只建议下一步提示词，不要自动执行下一步。
```
...
`paper-framework-figure-studio-pro` is a skill for making computer-science paper framework diagrams. It provides diverse candidate drafts for method overviews, architecture diagrams, pipeline/process figures, and agent workflows so authors can screen, compare, manually edit, and finalize the figure later. The current `main` branch has been updated to the **v3.2.15f** package and examples. This version is designed for ChatGPT Web and is recommended only for ChatGPT Web chat and work modes, noting that work mode is affected by token limits. Codex image gen (text) cannot accept image inputs, while the S5 stage in **v3.2.15f** needs to use images produced in stage S2 as references. A Cursor version and a ChatGPT chat-mode subagents collaboration version will be provided this week. The workflow explanation in this document still follows **v3.2.15b**. Older versions are archived in their corresponding Git tags. Special thanks to Xinyang Liu from Bristol for the support.

A Cursor version and a ChatGPT chat-mode subagents collaboration version will also be provided this week, with continued emphasis on independent auditing.
...
- Full older-version packages and complete examples are archived as Git tags and are no longer kept in the `main` branch directory. The v3.2.15c result archive and the v3.2.15b architecture figures remain available locally. Because aesthetic preference differs from person to person, some users may prefer earlier versions.
...
- Current main v3.2.15f skill package: `paper-framework-figure-studio-pro-v3.2.15f-skill.zip`
- Current v3.2.15f example paper and result directory: `example_semiDFL_v3.2.15f/`
- v3.2.15f example paper: `example_semiDFL_v3.2.15f/semiDFL.pdf`
- v3.2.15f ChatGPT Web gpt5.5 C candidates: `example_semiDFL_v3.2.15f/R2_results_chatgpt_web_v3.2.15_5.5eh/`, `C01.png` to `C04.png`
- v3.2.15f ChatGPT Web gpt5.5 F candidates: `example_semiDFL_v3.2.15f/R1_results_chatgpt_web_v3.2.15_5.5eh/`, `F01.png` to `F02.png`
- v3.2.15f ChatGPT Web chat-mode results (ChatGPT 5.6 Sol, extra high): `example_semiDFL_v3.2.15f/gptweb_chat_5.6sol_extrahigh/`, `F01.png` to `F02.png`
- v3.2.15f ChatGPT Web work-mode results (ChatGPT 5.6 Sol, extra high): `example_semiDFL_v3.2.15f/gptweb_work_5.6sol_extrahigh/`, `F01.png`, `F02.png`, and `conversation.png`
- v3.2.15c skill package: `paper-framework-figure-studio-pro-v3.2.15c-skill.zip`
- v3.2.15c ACM/IEEE/AAAI line-art result archive: `3.2.15c_result/`
- v3.2.15c ChatGPT web ACM/IEEE/AAAI line-art conversation screenshot: `3.2.15c_result/chatgpt_web_conversation_acm_ieee_aaai_line_art_v3.2.15c.png`
- v3.2.15c annotated S1/S4 screenshots showing the extra style requirement: `3.2.15c_result/S1_prompt_annotated_acm_ieee_aaai_line_art_v3.2.15c.jpg` and `3.2.15c_result/S4_prompt_annotated_acm_ieee_aaai_line_art_v3.2.15c.jpg`
- v3.2.15c ChatGPT web first-round ACM/IEEE/AAAI line-art candidates: `3.2.15c_result/R1_results_chatgpt_web_acm_ieee_aaai_line_art_v3.2.15c/`, `C01.png` to `C08.png`
- v3.2.15c Codex first-round ACM/IEEE/AAAI line-art candidates: `3.2.15c_result/R1_results_codex_acm_ieee_aaai_line_art_v3.2.15c/`, `C01.png` to `C08.png`
- v3.2.15c ChatGPT web second-round ACM/IEEE/AAAI line-art candidates: `3.2.15c_result/R2_results_chatgpt_web_acm_ieee_aaai_line_art_v3.2.15c/`, `F01.png` to `F06.png`
- v3.2.15c Codex second-round ACM/IEEE/AAAI line-art candidates: `3.2.15c_result/R2_results_codex_acm_ieee_aaai_line_art_v3.2.15c/`, `F01.png` to `F06.png`
