...
上图是 `example_semiDFL/result.png`，是本仓库随附 semiDFL 示例流程得到的最终论文（semiDFL)框架图. 这里特别感谢bristol的刘同学提供的素材支持。`example_semiDFL/semidfl-chatgpt-example.mhtml` 是 ChatGPT 网页版执行记录导出，可作为完整流程参考。为了方便对照快速使用，提供了示例中论文 PDF 文件 `example_semiDFL/semiDFL.pdf`。
